...
- **🤖 多模态 AI 智能导题引擎**：支持 `.docx`、`.pdf`、`.txt` 上传；Java 作为生产者入队 Redis Stream，**Python Worker**（`AiParser/`）通过 MinerU 解析文档并调用大模型（DeepSeek 等），经预览确认后幂等落库。
...
- **Python**: >= 3.10（运行 `AiParser` Worker；仅本地刷题/题库 CRUD 可不启）
...
**Python Worker**（复制 `AiParser/.env.example` 为 `.env`）：
...
│   ├── schema/              # 核心表 DDL
│   └── data/                # 种子数据、角色迁移脚本等
├── AiParser/           # AI 导入 Worker（MinerU + LLM，消费 Redis Stream）
├── src/
│   ├── main/
...
- 中文注释，且方法名具有高度可读性；
- 耗时第三方调用（MinerU、LLM 等）放在 **`AiParser` Worker** 或独立进程中，Java 侧通过 Redis Stream / 状态键协作，避免在 HTTP 请求线程内阻塞。