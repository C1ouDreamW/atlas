        ...
        try {
            stringRedisTemplate.execute((RedisCallback<Object>) connection -> {
                connection.streamCommands().xGroupCreate(
                        IShuaRedisCacheConstants.TASK_STREAM_KEY.getBytes(StandardCharsets.UTF_8),
                        IShuaRedisCacheConstants.TASK_STREAM_GROUP,
                        ReadOffset.latest(),
                        true);
                return null;
            });
            log.info("[Stream] consumer group created: {}", IShuaRedisCacheConstants.TASK_STREAM_GROUP);
        } catch (Exception e) {
            if (isBusyGroup(e)) {
                log.info("[Stream] consumer group already exists: {}", IShuaRedisCacheConstants.TASK_STREAM_GROUP);
                return;
            }
