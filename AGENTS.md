...
| `AI_IMPORT_CLEANUP_DEFAULT_OLDER_THAN_DAYS` | 7 | Admin 清理长时间未确认 PARSED 任务的默认天数 |
| `AI_IMPORT_REDIS_SYNC_ENABLED` | true | 是否启用 Redis → MySQL AI 任务状态同步 Job |
| `AI_IMPORT_REDIS_SYNC_INTERVAL_MS` | 30000 | Redis → MySQL AI 任务状态同步间隔（毫秒） |
...
```
前端上传文件
 → Java（生产者）：校验、落盘、写 MySQL task + Redis meta/status、XADD Redis Stream，返回 taskId
 → Python Worker（消费者）：XREADGROUP 拉任务 → MinerU 转 Markdown → LLM 解析 → 写 result/status
 → Java Sync Job：将 Redis status/result 同步到 MySQL ai_import_task（PARSED 预览可恢复）
...
| `GET` | `/api/v1/ai-import/tasks` | 分页查询当前用户 AI 导入任务，用于关页后恢复 |
| `GET` | `/api/v1/ai-import/tasks/{taskId}/status` | 轮询状态与预览 |
| `POST` | `/api/v1/question-banks/{bankId}/questions/batch` | 确认导入（SETNX 幂等） |
| `POST` | `/api/v1/admin/ai-import/tasks/cleanup` | Admin 清理长时间未确认的 PARSED 任务（默认 dryRun） |

关键约束：
- Stream：`ishua:task:stream`，消费组 `ishua-ai-workers`；Worker 在 `transf-python/` 独立进程运行
- 解析结果先写 `ishua:task:result:{taskId}`；Java 同步 Job 会将 PARSED 预览写入 MySQL `ai_import_task.preview_json` 作为可恢复权威源
...
| `ai_import_task` | AI 导入任务表（任务索引、状态、预览草稿） | `idx_user_status_time(user_id, status, submitted_at, is_deleted)`, `idx_parsed_expire(status, parsed_at, is_deleted)` |
